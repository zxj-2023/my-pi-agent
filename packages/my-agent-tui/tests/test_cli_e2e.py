...
async def test_cli_e2e_canonical_flow(tmp_path: Path):
    ...
    # 1. 验证启动横幅
    assert "my-agent-tui 终端交互助手" in out
    assert str(tmp_path) in out

    # 2. 验证 /help 命令分发及命令列表输出
    assert "my-agent-tui 可用命令" in out
    assert "/help" in out
    assert "/undo" in out
...
async def test_cli_e2e_multiturn_edit_diff_tasks_and_mcp(tmp_path: Path):
    ...
    # 验证文件真实被原地修改
    assert calc_py.read_text(encoding="utf-8") == "def add(a, b):\n    return a + b\n"
    ...
    assert "Query PostgreSQL database schema" in out
...
def test_top_level_package_exports():
    """验证 my_agent_tui 顶层导出符合公开规范契约。"""
    assert hasattr(my_agent_tui, "CommandDispatcher")
    assert callable(my_agent_tui.CommandDispatcher)

    assert hasattr(my_agent_tui, "EventRenderer")
    assert callable(my_agent_tui.EventRenderer)

    assert hasattr(my_agent_tui, "main")
    assert callable(my_agent_tui.main)

    all_exports = set(my_agent_tui.__all__)
    ...
    assert CommandDispatcher is my_agent_tui.CommandDispatcher
    assert EventRenderer is my_agent_tui.EventRenderer
    assert main is my_agent_tui.main
