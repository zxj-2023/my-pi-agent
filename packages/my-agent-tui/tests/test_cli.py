...
from my_agent_tui.cli import SLASH_COMMANDS, build_prompt_session, main, run_cli_loop
...
def test_module_python_m_help():
    result = subprocess.run(
        [sys.executable, "-m", "my_agent_tui", "--help"],
        capture_output=True,
        text=True,
        encoding="utf-8",
        check=False,
    ...
    assert "my-agent-tui 交互式终端助手" in result.stdout
