...
Planned two layers:
- `my_agent_core/` — framework layer (≈ `pig-agent-core`). Currently a minimal ReAct loop; being evolved per a 7-stage v1 roadmap (below).
...
Uses `uv` with a project-local `.venv` (never global pip). Run from the repo root.
...
Notes:
- `tests/` currently contains **no `.py` files** (only stale `__pycache__` from deleted LangChain/LangGraph experiments); pytest reports "no tests ran". Tests are meant to be written from the §7/§8 checklists in the design docs (see Roadmap), test-first.
...
- **`my_agent_core/tools.py`** — up-translation layer. `@tool` (支持 `name`/`description`/`params_model` 覆盖) builds a `Tool` class (function + JSON schema + pydantic 参数模型 + `to_openai_schema`/`execute`/`__call__`) from the function name, docstring, and type hints (pydantic 全集类型，允许默认值；无标注参数与 `*args`/`**kwargs` 在装饰时拒绝). `ToolResult` 承载执行结果（ok/data/error，永不抛）。
- **`my_agent_core/registry.py`** — `ToolRegistry`：工具注册表（register/unregister/get/get_schemas/execute），`execute` 收完整 `tool_call`（内部 JSON 解析 + 查表），全部错误转 `ToolResult`。
...
- **`my_agent_core/main.py`** — demo entry point: 3 example tools + 3 fixed questions; supplies its own `DEMO_SYSTEM_PROMPT`.
...
`my_agent_core/README.md` holds the authoritative **7-stage v1 plan**; `docs/superpowers/specs/2026-08-01-*.md` holds five design specs (framework / session / context / skills / extensibility). Current code is the pre-refactor `run_agent` version; the plan evolves it toward pi's three-piece shape:
