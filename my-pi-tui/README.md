...
- **大模型智能重试倒计时 (Auto-Retry)**：
  对标 Pi 原厂重试机制，当模型遭遇 429 或 5xx 瞬时错误时，输入框顶部边框呈现实时倒计时与错误说明（`── ⠸ 重试中 (1/3) 2.0s 后继续: 503... (Esc 取消) ──`），按 `Esc` 可毫秒级取消重试；
...
# 3. 运行 TUI 自动化测试套件 (71 tests, 100% 绿灯全通)
npm test
