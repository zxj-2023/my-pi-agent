...
深度对齐 Pi 原厂 resource-loader 规范，自动发现系统级、包级与工作区局部资源。
"""
...
def get_all_skill_dirs(workspace_path: Path, paths: AgentPaths | None = None) -> list[Path]:
    ...
    覆盖：
    1. 用户全局技能目录 (~/.my-pi-agent/skills, ~/.pi/agent/skills)
    2. 工作区局部技能目录 (<ws>/.skills, <ws>/.agents/skills, <ws>/.my-pi-agent/skills)
    3. Pi 包扩展技能目录 (~/.pi/agent/settings.json 中声明的 superpowers 等包)
    """
    home = Path.home()
        ...
        try:
            ...
            for pkg in data.get("packages", []):
                ...
                if pkg.startswith("git:"):
                    pkg_dir = home / ".pi" / "agent" / "git" / pkg[len("git:") :]
                elif pkg.startswith("npm:"):
                    pkg_dir = home / ".pi" / "agent" / "npm" / "node_modules" / pkg[len("npm:") :]
...
def scan_loaded_resources(
    ...
    """深度对标 Pi 原厂 showLoadedResources()，扫描并提取四大启动资源看板列表。"""
    ...
    # ── 1. Context 文件扫描 ──
    # 全局指令优先
    global_agents = home / ".pi" / "agent" / "AGENTS.md"
    if global_agents.is_file():
        context_files.append(_format_display_path(global_agents))
    elif (home / ".my-pi-agent" / "AGENTS.md").is_file():
        context_files.append(_format_display_path(home / ".my-pi-agent" / "AGENTS.md"))

    # 工作区指令
    for fname in ["AGENTS.override.md", "AGENTS.md", "CLAUDE.md"]:
        candidate = ws / fname
        if candidate.is_file() and str(candidate.resolve()) != str(global_agents.resolve()):
        ...
        try:
            ...
            for pkg in data.get("packages", []):
                ...
                if pkg.startswith("git:"):
                    ...
                    pkg_dir = home / ".pi" / "agent" / "git" / repo_path
                    label = repo_path.replace("github.com/", "")
                elif pkg.startswith("npm:"):
                    npm_name = pkg[len("npm:") :]
                    pkg_dir = home / ".pi" / "agent" / "npm" / "node_modules" / npm_name
                    label = npm_name
